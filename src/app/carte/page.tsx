    ...
    <main style={{ background: ISP.paper, minHeight: '100vh', color: ISP.ink }}>
      ...
      <div style={{
        ...
        <section style={{
          ...
          <p style={{ fontSize: 13.5, color: ISP.muted, margin: 0, lineHeight: 1.55 }}>
            Uploadez votre carte au format PDF — Ispalis extrait automatiquement les plats.
          </p>
          ...
          <label style={{
            ...
            border: `2px dashed ${importingPDF ? ISP.terracotta : ISP.rule}`,
            background: importingPDF ? `${ISP.terracotta}10` : ISP.paperWarm,
            cursor: importingPDF ? 'wait' : 'pointer',
            transition: 'all .2s',
            textAlign: 'center',
          }}>
            <BottleI color={importingPDF ? ISP.terracotta : ISP.muted} size={28} />
            <div style={{ fontWeight: 800, fontSize: 14, color: ISP.ink }}>
              {importingPDF ? 'Extraction en cours…' : 'Cliquez ou glissez votre PDF'}
            </div>
            <div style={{ fontSize: 12, color: ISP.muted }}>
              Format PDF · Jusqu&apos;à 30 plats détectés
            </div>
            <input
        ...
        <section style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              background: platsRemplis > 0 ? ISP.sagePale : ISP.paper,
              color: platsRemplis > 0 ? ISP.sage : ISP.muted,
          ...
          {plats.length === 0 ? (
            <div style={{
              ...
              textAlign: 'center', color: ISP.muted, fontSize: 13.5, lineHeight: 1.55,
            }}>
              Aucun plat pour l&apos;instant.<br />
              Importez un PDF à gauche, ou ajoutez vos plats ci-dessous.
            </div>
          ) : (
            <div style={{
          ...
          <button
            ...
            style={{
              padding: '10px 14px', borderRadius: 10,
              border: `1.5px dashed ${ISP.terracotta}`,
              background: 'transparent', color: ISP.terracotta,
      ...
      {/* ─── Generate CTA */}
      {plats.length > 0 && (
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 44px 24px' }}>
          <button
            onClick={genererAccords}
            disabled={loading || platsRemplis === 0}
            style={{
              width: '100%',
              background: (loading || platsRemplis === 0) ? ISP.muted : ISP.burgundy,
              color: ISP.card, border: 'none', borderRadius: 14,
              padding: '18px 24px',
              fontFamily: 'inherit', fontWeight: 800, fontSize: 16,
              cursor: (loading || platsRemplis === 0) ? 'not-allowed' : 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16,
              boxShadow: '0 10px 28px -12px rgba(94,17,25,.45)',
              transition: 'all .2s',
          ...
          >
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 12 }}>
              ...
              {loading
                ? 'Génération en cours…'
                : `Générer les accords pour ${platsRemplis} plat${platsRemplis > 1 ? 's' : ''}`}
            </span>
            {!loading && (
      ...
      {/* ─── Results */}
      {(accords.length > 0 || alertes.length > 0) && (
        <section style={{ padding: '20px 44px 80px', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{
            display: 'flex', alignItems: 'center', gap: 14,
            fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase',
...
function DishRow({ index, value, onChange, onDelete }: {
  index: number; value: string; onChange: (v: string) => void; onDelete: () => void
}) {
  const filled = value.trim().length > 0
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 10,
      background: ISP.paperWarm, borderRadius: 10,
      padding: '4px 6px 4px 12px',
      border: `1px solid ${filled ? `${ISP.terracotta}40` : ISP.rule}`,
      transition: 'all .15s',
    }}>
      <span style={{
        fontSize: 13, fontWeight: 800, color: filled ? ISP.burgundy : ISP.muted,
        minWidth: 20, fontVariantNumeric: 'tabular-nums',
      }}>{index}.</span>
      <input
        type="text" value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder={`Plat ${index}`}
        style={{
          flex: 1, minWidth: 0, border: 'none', outline: 'none', background: 'transparent',
          padding: '8px 0', fontFamily: 'inherit',
          fontSize: 14, fontWeight: 600, color: ISP.ink,
        }}
      />
      <button
        onClick={onDelete}
