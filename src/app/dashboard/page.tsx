    ...
    <main style={{ maxWidth: 900, margin: '0 auto', padding: 24, backgroundColor: '#ffffff', minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, borderBottom: '2px solid #5E1119', paddingBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button onClick={() => router.push('/carte')} style={{ padding: '8px 16px', background: 'transparent', border: '1px solid #5E1119', color: '#5E1119', borderRadius: 4, cursor: 'pointer' }}>
            Carte
          </button>
      ...
      {stock.length > 0 && (
        <div style={{ background: '#F5E6E8', borderRadius: 8, padding: 10, marginBottom: 20, fontSize: 13, color: '#5E1119' }}>
          🍷 <strong>{stock.length} références</strong> en cave — utilisées en priorité pour vos accords
        </div>
      )}

      <h2 style={{ marginBottom: 8, color: '#1a1a1a' }}>Accords rapides</h2>
      <p style={{ color: '#666', marginBottom: 16, fontSize: 13 }}>Saisissez quelques plats pour générer des accords ponctuels. Pour importer votre carte complète, utilisez le bouton Carte.</p>

      <div style={{ background: '#f9f9f9', padding: 20, borderRadius: 8, marginBottom: 24 }}>
        {plats.map((plat, i) => (
          <div key={i} style={{ marginBottom: 10 }}>
            <input type="text" placeholder={`Plat ${i + 1} — ex: Magret de canard`} value={plat}
              onChange={e => updatePlat(i, e.target.value)}
              style={{ width: '100%', padding: 10, border: '1px solid #ddd', borderRadius: 4, fontSize: 14, boxSizing: 'border-box', color: '#1a1a1a', backgroundColor: 'white' }} />
          </div>
        ))}
        {erreur && <p style={{ color: 'red', marginBottom: 8, fontSize: 13 }}>{erreur}</p>}
        <button onClick={genererAccords} disabled={loading}
          style={{ width: '100%', padding: 12, background: '#5E1119', color: 'white', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 15, marginTop: 4 }}>
