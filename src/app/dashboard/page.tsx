...
export default function Dashboard() {
  ...
  const genererAccords = async () => {
    ...
    if (platsRemplis.length === 0) {
      setErreur('Ajoutez au moins un plat')
      return
    }
    setLoading(true)
    setErreur('')
    ...
    try {
      const response = await fetch('/api/recommandations', {
        ...
        body: JSON.stringify({ plats: platsRemplis, stock: [], ton: 'professionnel' })
      })
  ...
  return (
    <main style={{ maxWidth: 800, margin: '0 auto', padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, borderBottom: '2px solid #5E1119', paddingBottom: 16 }}>
        ...
        <button onClick={handleDeconnexion} style={{ padding: '8px 16px', background: 'transparent', border: '1px solid #5E1119', color: '#5E1119', borderRadius: 4, cursor: 'pointer' }}>
          Déconnexion
        </button>
      </div>
      <h2 style={{ marginBottom: 16 }}>Menu du jour</h2>
      ...
      <div style={{ background: '#f9f9f9', padding: 20, borderRadius: 8, marginBottom: 24 }}>
        {plats.map((plat, i) => (
          <div key={i} style={{ marginBottom: 12 }}>
            <input
              type="text"
              placeholder={`Plat ${i + 1} — ex: Magret de canard aux cerises`}
              value={plat}
        ...
        <button
          ...
          disabled={loading}
          style={{ width: '100%', padding: 14, background: '#5E1119', color: 'white', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 16, marginTop: 8 }}
        >
          ...
          {resultats.accords && resultats.accords.map((accord: any, i: number) => (
            <div key={i} style={{ background: 'white', border: '1px solid #eee', borderRadius: 8, padding: 20, marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
                {[
                  ...
                  <div key={label} style={{ background: '#f9f9f9', borderRadius: 6, padding: 12, borderTop: `3px solid ${color}` }}>
                    ...
                    <div style={{ fontWeight: 600, marginBottom: 4 }}>{data.vin}</div>
              ...
              <div style={{ background: '#F0F7F0', borderRadius: 6, padding: 12, borderLeft: '3px solid #2E7D32' }}>
                ...
                <div style={{ fontWeight: 600, marginTop: 4 }}>{accord.accord_sans_alcool.boisson}</div>
